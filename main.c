...
// running through the whole array and setting each cell to a combination of its neighbors. 
// It does this several times; the more iterations it does, the more accurate the results, 
// but the slower things run. In the step function above, four iterations are used. After each iteration, 
...
// but it does some more running through the data and setting values, with some calls to lin_solve thrown in for fun.
...
// This function is responsible for actually moving things around. To that end, it looks at each cell in turn. 
// In that cell, it grabs the velocity, follows that velocity back in time, and sees where it lands. 
// It then takes a weighted average of the cells around the spot where it lands, then applies that value to the current cell.
